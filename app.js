...
const ALLOW_SAMPLE_FALLBACK = CONFIG.ALLOW_SAMPLE_FALLBACK === true;
const REQUIRE_READY_STATUS = CONFIG.REQUIRE_READY_STATUS !== false;
const MAX_COMPARE = Number(CONFIG.MAX_COMPARE || 4);
...
function normalizeRows(rows) {
  return (rows || [])
    ...
    .filter(row => {
      const readyStatus = String(get(row, "Public Data Status") || "").toLowerCase();
      if (!REQUIRE_READY_STATUS || !readyStatus) return true;
...
function resetStats() {
  els.count.textContent = "0 unit options";
  els.propertyCount.textContent = "0";
  els.minRate.textContent = "—";
  els.petCount.textContent = "0";
}
...
function renderLoadError(error) {
  ...
  els.grid.innerHTML = `
    <div class="empty error-state">
      <strong>Catalogue data could not be loaded.</strong>
      <span>Please refresh the page. If the issue continues, contact the Office of Finance so the data connection can be checked.</span>
    </div>
  `;
}
...
async function loadData() {
  ...
  if (!url || url.includes("PASTE_YOUR")) {
    if (ALLOW_SAMPLE_FALLBACK) return samplePayload();
    throw new Error("Live catalogue data source is not configured in config.js.");
  }

  let payload;

  // Apps Script usually works most reliably through JSONP from GitHub Pages.
  if (isAppsScriptUrl(url)) {
    try {
      payload = await withTimeout(loadFromJsonp(url), 9000, "Apps Script JSONP");
    } catch (jsonpError) {
      console.warn("JSONP failed. Trying fetch fallback.", jsonpError);
      payload = await withTimeout(loadFromFetch(url), 9000, "Apps Script fetch");
    }
  } else {
    try {
      payload = await withTimeout(loadFromFetch(url), 9000, "Data fetch");
    } catch (fetchError) {
      console.warn("Fetch failed. Trying JSONP fallback.", fetchError);
      payload = await withTimeout(loadFromJsonp(url), 9000, "JSONP fallback");
    }
  }

  if (!payload || payload.ok === false) {
    throw new Error((payload && payload.error) || "Catalogue API returned an invalid response.");
  }
...
Cancellation: ${escapeHtml(group.cancellation || "Not listed")}</p>
          </div>
...
function toggleCompare(id) {
  ...
  if (selectedForCompare.has(id)) {
    ...
    if (selectedForCompare.size >= MAX_COMPARE) {
      alert(`Compare up to ${MAX_COMPARE} hotels at a time.`);
      return;
    }
...
async function init() {
  els.grid.innerHTML = `<div class="empty">Loading live catalogue data…</div>`;
  els.updatedAt.textContent = "Loading live data…";
  resetStats();
  ...
  try {
    ...
    if (payload.isSample) {
      els.updatedAt.textContent = "Preview data only — live source not connected.";
    } else {
